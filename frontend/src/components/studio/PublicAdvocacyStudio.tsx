    ...
    try {
      const res = await fetch('/api/badges', {
        ...
        body: JSON.stringify({
          ...
          name: badge.name, // The first field they filled out
          avatarUrl: dataUrl,
          themeStyle: themeId,
          customFrameUrl: frameUrl,
          dynamicData: dynamicForm // Send ALL custom fields to the database!
        })
      });
      ...
      const handleCustomFormSubmit = (e: React.FormEvent) => {
        ...
        // Automatically map the first active field to the Canvas "Name" and the second to the Canvas "Subtext"
        ...
        onUpdateBadge({
          name: primaryText,
          title: secondaryText,
          company: '', // Optional: you can leave these blank as Canvas pulls from 'title' now
        });
                ...
                <form onSubmit={handleCustomFormSubmit} className="p-6 sm:p-8 space-y-5">
                  ...
                  {formFields.filter((f: any) => f.show).length === 0 && (
                    <div className="text-center py-6 text-sm text-slate-500 font-medium">
                      No registration fields required.
                    </div>
                  )}
